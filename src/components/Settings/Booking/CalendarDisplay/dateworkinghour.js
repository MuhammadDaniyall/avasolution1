...
const WorkingTimeForm = (props) => {
  ...
  useEffect(() => {
    if (visible) {
      if (modalData) {
        const tmpModalData = JSON.parse(modalData);
        if (tmpModalData != undefined) {
          if(tmpModalData.Data.GeneralHours.isWorking != "Working"){
            tmpModalData.Data.GeneralHours.data.push(['9:00 am', '6:00 pm', 'none', 'none']);
          }
          setWorkingTime(tmpModalData.Data.GeneralHours);
          if (tmpModalData.Data.WaitListHours) {
            if(tmpModalData.Data.WaitListHours.isWorking != "Working"){
              tmpModalData.Data.WaitListHours.data.push(['9:00 am', '6:00 pm', 'none', 'none']);
            }
            setWaitListTime(tmpModalData.Data.WaitListHours);
            setWaitlistIsGeneral(false);
          } else {
            setWaitListTime(data1[date.day - 1]);
          }
          if (tmpModalData.Data.BookingHours) {
            if(tmpModalData.Data.BookingHours.isWorking != "Working"){
              tmpModalData.Data.BookingHours.data.push(['9:00 am', '6:00 pm', 'none', 'none']);
            }
            setBookingTime(tmpModalData.Data.BookingHours);
        ...
        <TabPane tab="General hours" key="working">
          <Form autoComplete="off">
            <Row>
              ...
              <Col span={5}>
                <Form.Item defaultValue={workingTime.isWorking}>
                  <Select
                    value={workingTime.isWorking}
                    style={{ width: 140 }}
          ...
          {!waitlistIsGeneral && (
            <Form autoComplete="off">
              <Row>
                ...
                <Col span={5}>
                  <Form.Item defaultValue={waitListTime.isWorking}>
                    <Select
                      defaultValue={waitListTime.isWorking}
                      style={{ width: 140 }}
                      onChange={(e) => handelChangeWorkingSelect(e)}
                    >
                      <Option value="Working">Working</Option>
          ...
          {!bookingIsGeneral && (
            <Form autoComplete="off">
              <Row>
                ...
                <Col span={5}>
                  <Form.Item defaultValue={BookingTime.isWorking}>
                    <Select
                      defaultValue={BookingTime.isWorking}
                      style={{ width: 140 }}
                      onChange={(e) => handelChangeWorkingSelect(e)}
                    >
                      <Option value="Working">Working</Option>
