...
const WorkingTimeForm = (props) => {
  ...
  useEffect(() => {
    if (visible) {
      const modalData1 = modalData;
      if(modalData1)
      {
        if (modalData1.openingGeneralHours !== "0") {
          setWorkingTime(JSON.parse(modalData1.openingGeneralHours));
          if (modalData1.openingWaitListHours !== "0") {
            setWaitListTime(JSON.parse(modalData1.openingWaitListHours));
            setWaitlistIsGeneral(false);
          }
          if (modalData1.openingBookingHours !== "0") {
            setBookingTime(JSON.parse(modalData1.openingBookingHours));
  ...
  const save = async() => {
    ...
    tempWorking.map((item) => {
      if (item.isWorking === 'Working') {
        ...
        item.data = [['9:00 am', '6:00 pm', 'none', 'none']];
      }
      return true;
    });
    ...
    if (isValid) {
      await saveWorkingTime(workingTime, waitListTime, BookingTime, waitlistIsGeneral, bookingIsGeneral);
                        ...
                        {item.data.map((val, indexVal) => {
                          return (
                            <Space style={{ display: '-webkit-box' }}>
                              <Form.Item style={{ boxSizing: 'inherit' }} key={indexVal}>
                                <TimePicker
                                  clearIcon={false}
                                  showNow={false}
                                  style={{ width: 140 }}
                      ...
                      <Col>
                        <Form.Item key={index}> 
                          {item.data.map((val, indexVal) => {
                            return (
                              <Space style={{ display: 'flex' }}>
                                <Form.Item key={indexVal} style={{ boxSizing: 'inherit' }}>
                                  <TimePicker
                                    clearIcon={false}
                                    showNow={false}
                                    style={{ width: 140 }}
