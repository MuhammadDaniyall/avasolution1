// invited resource's signup page

import React, { useState, useEffect } from 'react';
import { Button, message, Input } from 'antd';
import { connect } from 'react-redux';
import { Link, Redirect } from 'react-router-dom';
import { userSignUp, resourceConfirmById, resourceSignUp } from '../../appRedux/actions/Auth';
...
const Signup = (props) => {
  ...
  const resourceConfirmById = async (id) => {
    await props.resourceConfirmById({ id: props.match.params.ID });
  };
  ...
  const handleClick = async () => {
    const name = props.resourceConfirm.name;
    const email = props.resourceConfirm.email;
    await props.resourceSignUp({
      name: name,
      email: email,
  ...
  if (props.resourceConfirm === 'resource already signup' || props.resourceConfirm === 'resource not found') {
    message.warning(props.resourceConfirm);
      ...
      <div className="gx-login-wrapper">
        ...
        <div className="gx-login-input-wrapper">
          <Input
            ...
            value={props.resourceConfirm ? props.resourceConfirm.email : ''}
...
const mapStateToProps = ({ auth }) => {
  const { token, authMsg, resourceConfirm } = auth;
  return { token, authMsg, resourceConfirm };
};
