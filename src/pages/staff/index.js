...
const CustomerScreen = (props) => {
  ...
  const { user, staffData, getStaff, addStaff, updateCustomer, deleteStaff } = props;
  ...
  const handleSubmit = async (values) => {
    if (formdata === null) await addStaff({ ...values, user_id: user.id });
...
const mapStateToProps = ({ auth, staff }) => {
  const { user } = auth;
